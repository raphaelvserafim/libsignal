...
describe('keyhelper', () => {
  ...
  describe('generateSignedPreKey', () => {
    ...
    it('should throw on invalid identity key pair', () => {
      expect(() => keyhelper.generateSignedPreKey({ pubKey: Buffer.alloc(10), privKey: Buffer.alloc(10) }, 1)).toThrow(
        'Invalid argument for identityKeyPair',
      )
    })

    it('should throw on negative keyId', () => {
      const identityKeyPair = keyhelper.generateIdentityKeyPair()
      expect(() => keyhelper.generateSignedPreKey(identityKeyPair, -1)).toThrow('Invalid argument for signedKeyId')
    })
  })
