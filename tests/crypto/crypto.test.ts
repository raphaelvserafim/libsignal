...
describe('crypto', () => {
  describe('encrypt / decrypt', () => {
    ...
    it('should throw on wrong key length', () => {
      expect(() => crypto.encrypt(Buffer.alloc(16), Buffer.from('test'), Buffer.alloc(16))).toThrow(
        'Key must be 32 bytes',
      )
    })

    it('should throw on wrong IV length', () => {
      expect(() => crypto.encrypt(Buffer.alloc(32), Buffer.from('test'), Buffer.alloc(8))).toThrow(
        'IV must be 16 bytes',
      )
    })
  ...
  describe('deriveSecrets', () => {
    ...
    it('should throw on wrong salt length', () => {
      expect(() => crypto.deriveSecrets(Buffer.alloc(32), Buffer.alloc(16), Buffer.from('info'))).toThrow(
        'salt of incorrect length',
      )
    })

    it('should throw on invalid chunks', () => {
      expect(() => crypto.deriveSecrets(Buffer.alloc(32), Buffer.alloc(32), Buffer.from('info'), 4)).toThrow(
        'Chunks must be between 1 and 3',
      )
    })
