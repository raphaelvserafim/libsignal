...
/** Safely convert various key formats to Buffer */
function toBuffer(key: Uint8Array | Buffer | string | { type: string; data: number[] }): Buffer {
  ...
  } else if (key && typeof key === 'object' && 'data' in key && Array.isArray(key.data)) {
    // BufferJSON format: { type: 'Buffer', data: [...] }
    return Buffer.from(key.data)
  }

  throw new Error(`Cannot convert signing key to Buffer: unexpected type ${typeof key}`)
}
...
export class SenderKeyState {
  ...
  public getSigningKeyPublic(): Buffer {
    return toBuffer(this.senderKeyStateStructure.senderSigningKey.public)
  }
