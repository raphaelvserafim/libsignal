...
function validatePrivKey(privKey: Uint8Array): void {
  ...
  if (!(privKey instanceof Uint8Array)) {
    throw new Error(
      `Invalid private key type: ${privKey && typeof privKey === 'object' ? (privKey as any).constructor?.name : typeof privKey}`,
    )
  }
...
function scrubPubKeyFormat(pubKey: Uint8Array): Buffer {
  if (!(pubKey instanceof Uint8Array)) {
    throw new Error(
      `Invalid public key type: ${pubKey && typeof pubKey === 'object' ? (pubKey as any).constructor?.name : typeof pubKey}`,
    )
  }
...
export function generateKeyPair(): { pubKey: Buffer; privKey: Buffer } {
  try {
    ...
    const privKey = Buffer.from(
      privateDerBytes.slice(PRIVATE_KEY_DER_PREFIX.length, PRIVATE_KEY_DER_PREFIX.length + 32),
    )
...
export function verifySignature(pubKey: Uint8Array, msg: Uint8Array, sig: Uint8Array): boolean {
  const scrubbed = scrubPubKeyFormat(pubKey)
  ...
  return curveJs.verify(scrubbed, msg, sig)
}
